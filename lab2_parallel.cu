...
#define RADIUS 5        // Define the radius of the kernel
#define KERNEL_SIZE (2 * RADIUS + 1)  // Kernel size (e.g., for RADIUS = 5, KERNEL_SIZE = 11)
...
#define N 1024  // Size of the input array (updated to 1024)
int main() {
    ...
    // Initialize input array (e.g., all values set to 1)
    for (int i = 0; i < N; i++) {
        h_in[i] = 1;
    }

    // Initialize kernel (e.g., [1, 2, 3, 2, 1] for RADIUS = 5)
    for (int i = 0; i < KERNEL_SIZE; i++) {
        h_kernel[i] = i + 1;  // Example: Linear kernel [1, 2, 3, 2, 1, ...]
    }
